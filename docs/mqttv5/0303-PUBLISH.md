...
-    如果保留消息处理属性(RetainHandling)被设置为0，服务端**必须**发送主题与客户端订阅的主题过滤器（Topic Filter）相匹配的所有保留消息 \[MQTT-3.3.1-9\]。
-    如果保留消息处理属性被设置为1，且尚不存在匹配的订阅，服务端**必须**发送主题与客户端订阅的主题过滤器相匹配的所有保留消息。如果已存在相匹配的订阅，服务器**不能**发送这些保留消息 \[MQTT-3.3.1-10\]。
